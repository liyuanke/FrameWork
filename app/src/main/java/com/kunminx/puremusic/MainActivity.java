...
public class MainActivity extends BaseActivity<MainActivityViewModel> {
    ...
    public void addBindingParam(DataBindingConfig dataBindingConfig) {
        dataBindingConfig.addBindingParam(BR.event, new EventHandler());
    }

    @Override
    public void initView() {
        mViewModel.getNetLoadChangeObservable().maskLoad.postValue(new Mask(true));
        postDelayed(new Runnable() {
            @Override
            public void run() {
                mViewModel.getNetLoadChangeObservable().maskLoad.postValue(new Mask(true));
                mViewModel.getNetLoadChangeObservable().maskLoad.setValue(new Mask(false));
            }
        },4000);
    }
